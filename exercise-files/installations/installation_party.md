...
In this class, we will be using [Visual Studio Code or VS Code](https://code.visualstudio.com/download) to work with in Python. It is aa good choice because it's free, widely used (and thus less likely to deprecate), and intuitive to use with its defaults.

### For Jupyter Notebooks
In this class, we will be using the [Jupyter Notebooks extension](https://code.visualstudio.com/docs/datascience/jupyter-notebooks) in VS Code to run Python code, build virtual environments, and install libraries. You should install the extension using the following steps: 

1. Open VS Code.
2. Go to `Code > Settings > Extensions` in your VS Code or use the shortcut Ctrl+Shift+X (Windows/Linux) or Cmd+Shift+X (macOS). This should pop up a window on the left. 
